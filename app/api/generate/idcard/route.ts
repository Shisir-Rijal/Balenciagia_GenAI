...
export async function POST(req: NextRequest) {
  let body: { image?: string; guestCode?: string; designation?: string };
  try {
    body = await req.json();
  } catch {
  ...
  const { image, guestCode, designation } = body;
  ...
  // 2. Load workflow and inject uploaded image
  // Colleague: replace __INPUT_IMAGE__ with the LoadImage node filename field in your workflow
  let workflow: Record<string, unknown>;
  try {
    const raw = readFileSync(join(process.cwd(), "workflows", "idcard.json"), "utf-8");
    const replaced = raw
